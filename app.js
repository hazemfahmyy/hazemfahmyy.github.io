...
function initStoryCanvas() {
    ...
    function draw() {
        ...
        if (currentStoryStep === 1) { 
            // ----------------------------------------------------
            // PHASE 1: MPC KINEMATICS (Fixed Tracking without Oscillation)
            // ----------------------------------------------------
            const getRoadY = (x, t) => ch/2 + Math.sin(x * 0.005 - t * 2) * 70;
            
            // Outer Road Boundaries
            ctx.beginPath(); ctx.strokeStyle = 'rgba(255, 255, 255, 0.15)'; ctx.lineWidth = 3;
            for(let x=0; x<cw; x+=20) { let ry = getRoadY(x, time); x===0 ? ctx.moveTo(x, ry-50) : ctx.lineTo(x, ry-50); }
            for(let x=0; x<cw; x+=20) { let ry = getRoadY(x, time); x===0 ? ctx.moveTo(x, ry+50) : ctx.lineTo(x, ry+50); }
            ctx.stroke();
            
            // Target Centerline
            ctx.beginPath(); ctx.setLineDash([10,15]); ctx.strokeStyle = 'rgba(16, 185, 129, 0.5)';
            for(let x=0; x<cw; x+=20) { let ry = getRoadY(x, time); x===0 ? ctx.moveTo(x, ry) : ctx.lineTo(x, ry); }
            ctx.stroke(); ctx.setLineDash([]);

            // Ego-Vehicle (Smooth tracking, NO oscillation)
            const carX = cw * 0.25; 
            const carY = getRoadY(carX, time); // Perfectly pinned to the path
            const nextY = getRoadY(carX + 10, time);
            const headingAngle = Math.atan2(nextY - carY, 10);

            // Draw Horizon Projection
            ctx.beginPath(); 
            ...
            for(let step=1; step<=20; step++) {
                simX += 12; 
                ctx.lineTo(simX, getRoadY(simX, time));
            }
            ctx.stroke();
            ...
            ctx.fillText("EGO_BOX", carX - 20, carY - 20);
            ctx.fillStyle = '#f59e0b';
            ctx.fillText("HORIZON_N20", carX + 50, carY - 40);
            ...
            // YOLO SEGMENTATION OVERLAYS (Boxes + Semantic Polygons)
            
            // 1. Car Segmentation Overlay (Cyan)
            ctx.fillStyle = 'rgba(14, 165, 233, 0.4)'; // Semantic Mask
            ctx.beginPath();
            ctx.moveTo(carX, carY+15); ctx.lineTo(carX+20, carY); ctx.lineTo(carX+70, carY); ctx.lineTo(carX+90, carY+15); ctx.lineTo(carX+90, carY+35); ctx.lineTo(carX, carY+35);
            ctx.fill();
            
            ctx.strokeStyle = '#0ea5e9'; ctx.lineWidth = 2;
            ctx.strokeRect(carX-2, carY-2, 94, 44); // Bounding Box
            ctx.fillStyle = '#0ea5e9'; ctx.fillRect(carX-2, carY-18, 70, 16); // Label BG
            ctx.fillStyle = '#fff'; ctx.font = "bold 10px monospace"; ctx.fillText("car 0.98", carX+2, carY-6);

            // 2. Truck Segmentation Overlay (Orange)
            ctx.fillStyle = 'rgba(245, 158, 11, 0.4)'; // Semantic Mask
            ctx.beginPath();
            ctx.moveTo(tX, tY+10); ctx.lineTo(tX+30, tY+10); ctx.lineTo(tX+40, tY); ctx.lineTo(tX+120, tY); ctx.lineTo(tX+120, tY+40); ctx.lineTo(tX, tY+40);
            ctx.fill();

            ctx.strokeStyle = '#f59e0b'; ctx.lineWidth = 2;
            ctx.strokeRect(tX-2, tY-2, 124, 49); // Bounding Box
            ctx.fillStyle = '#f59e0b'; ctx.fillRect(tX-2, tY-18, 85, 16); // Label BG
            ...
            const layers = [5, 7, 7, 2]; // Nodes per layer
            const layerSpacing = cw / (layers.length + 1);
            const nodeSpacing = 35;
            
            let nodes = [];
            
            // Build Nodes & Assign LRP Relevance Scores
            layers.forEach((count, i) => {
                for(let j=0; j<count; j++) {
                    nodes.push({
                        ...
                        // Hardcode the specific fault trace route (Red path)
                        relevance: (i === 3 && j === 0) ? 1.0 : // Output Node (Fault)
                                   (i === 2 && (j === 2 || j === 4)) ? 0.8 :
                                   (i === 1 && (j === 3 || j === 5)) ? 0.6 :
                                   (i === 0 && j === 1) ? 1.0 : 0.0 // Root Cause Feature
                    });
                }
            });

            // Draw Edges (Forward Pass + Backward Relevance)
            nodes.forEach(n1 => {
                nodes.forEach(n2 => {
                    if (n2.layer === n1.layer + 1) {
                        ...
                        // Check if this edge is part of the relevance trace
                        ...
                        if (isRelevanceEdge) {
                            // Active LRP Backprop Path (Pulsing Red)
                            ctx.strokeStyle = `rgba(239, 68, 68, ${0.4 + Math.sin(time*6)*0.4})`;
                            ctx.lineWidth = 3;
                        } else {
                            // Standard Inactive Neuron Path
                            ctx.strokeStyle = 'rgba(255, 255, 255, 0.05)';
                            ctx.lineWidth = 1;
                        }
                        ctx.stroke();
