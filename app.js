...
function initBgCanvas() {
    ...
    const myBgToken = bgLoopToken; // Capture instance identifier
    ...
    function renderBg() {
        if (myBgToken !== bgLoopToken) return; // Self-destruct if new page instance loads
...
function initStoryCanvas() {
    ...
    const myStoryToken = storyLoopToken; // Capture unique token instance
    ...
    function draw() {
        if (myStoryToken !== storyLoopToken) return; // Forcefully kill legacy duplicate loops instantly
        ...
        if (currentStoryStep === 1) { 
            // ----------------------------------------------------
            // PHASE 1: MPC KINEMATICS (Rock-Solid Grounded Tracking)
            // ----------------------------------------------------
            const getRoadY = (x) => ch/2 + Math.sin(x * 0.008) * 45 + Math.cos(x * 0.004) * 20;
            
            // Outer Road Boundaries (100% Static Coordinates)
            ...
            // Target Centerline (Animate dash offset only to show progress flow smoothly)
            ctx.save();
            ctx.beginPath(); 
            ...
            // Ego-Vehicle Tracking Coordinates
            const carX = cw * 0.25; 
            const carY = getRoadY(carX); 
            const nextY = getRoadY(carX + 10);
            const headingAngle = Math.atan2(nextY - carY, 10);

            // Draw MPC Horizon Preview Path
            ctx.beginPath(); 
            ...
            // Diagnostic Layout HUD Labels
            ctx.fillStyle = '#0ea5e9'; ctx.font = "10px monospace"; 
            ctx.fillText("EGO_ACTUATOR_NODE", carX - 30, carY - 22);
            ctx.fillStyle = '#f59e0b';
            ctx.fillText("MPC_PREDICTION_HORIZON (N=25)", carX + 40, carY - 35);
